...
def benchmark(trajectory, indices, n_frames):
    ...
    for j, i in enumerate(range(start, stop)):
        # input/output time
        with timeit() as io:
            pos = f['particles/trajectory/position/value'][i]
        total_io += io.elapsed
        # rmsd calculation time
        with timeit() as rms:
            rmsd_array[j] = rmsd(pos[indices], x_ref, superposition=True)
        total_rmsd += rms.elapsed

    f.close()

    block_times = np.array((rank, t_init, t_init_h5py,
                            total_io, total_io/bsize,
                            total_rmsd, total_rmsd/bsize, 0., 0.),
                            dtype=float)
    ...
    times_buffer = None
    if rank == 0:
        times_buffer = np.empty(9*size, dtype=float)
    comm.Gather(sendbuf=block_times, recvbuf=times_buffer, root=0)

    if rank == 0:
        # turn 1 dimensional vector into size x 9 matrix where the
        # columns are t_init, t_rmsd, etc and rows are each rank
        times_buffer = times_buffer.reshape(size,9)
...
if __name__ == "__main__":
    ...
    if rank == 0:
        data_path = '/scratch/ejakupov/parallel_h5md/data/gen2/Agave/rmsd/90Kframe/'
