...
def benchmark(topology, trajectory, indices):
    """Benchmarks rmsd calculation for a given universe"""
    with timeit() as init:
        with timeit() as init_top:
            u = mda.Universe(topology)
        with timeit() as init_traj:
            u.load_new(trajectory, driver="mpio", comm=comm, sub=indices)

        pos = u.trajectory.ts.positions
        x_ref = pos.copy()
    ...
    for j, i in enumerate(range(start, stop)):
        ...
        with timeit() as rms:
            rmsd_array[j] = rmsd(pos, x_ref, superposition=True)
...
if __name__ == "__main__":
    ...
    indices = mda.Universe('/scratch/ejakupov/parallel_h5md/datafiles/YiiP_system.pdb').select_atoms('protein and name CA').indices

    topology, trajectory = (args.test_top, args.test_traj)

    times_array, rmsd_array = benchmark(topology, trajectory, indices)

    if rank == 0:
        data_path = '/scratch/ejakupov/parallel_h5md/data/Agave/gen3/rmsd/90Kframe/'
